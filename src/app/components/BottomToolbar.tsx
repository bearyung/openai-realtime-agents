...
interface BottomToolbarProps {
  ...
  setIsCustomerUI: (val: boolean) => void;
}
...
function BottomToolbar({
  ...
  return (
    <div className="p-4 flex flex-row items-center justify-center gap-x-8 relative">
      {!isCustomerUI ? (
        <>
          <button
      ...
      {/* Customer UI Toggle Button - Always visible on bottom right */}
      <button
        onClick={() => setIsCustomerUI(!isCustomerUI)}
        className="absolute right-4 bg-gray-700 hover:bg-gray-800 text-white px-4 py-2 rounded-md cursor-pointer transition-colors"
        title={isCustomerUI ? "Switch to Debug UI" : "Switch to Customer UI"}
      >
        {isCustomerUI ? "Debug UI" : "Customer UI"}
      </button>
    </div>
  );
}
